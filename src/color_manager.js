 ...
 * @name color_manager
 */
export const color_manager = function () {

    const controller = constants.CONTROLLER;
    let current_hsv, current_rgb, current_left_right = 0, current_up_down = 0;
    const steps = 0, gradient_steps = 8, that = {
    ...
    const init = function (params) {
        ...
        $(bg_image).load(function () {
            ...
            //create drag element
            const coords = $("#" + that.color_bg_id).offset();
            const dragger = $("<div>").addClass("drag").attr("id", that.drag_element_id).appendTo("#" + that.color_bg_id).draggable({
                grid: [10, 0],
                cursorAt: {
                    left: 5
                },
            ...
            let left_right, up_down, max, min;


            const up_down_steps = 100 / gradient_steps;
    ...
    var _show_color_variations = function () {
        ...
        for (let i = 0; i < numbers.length; i++) {
            ...
            if (show) {
                ...
                const rgb = HSBToRGB(hsv_variations);

                const li = $("<li />").css("background", "#" + RGBToHex(rgb)).html("&nbsp;").appendTo("#color_var").bind("click", {
                    "value": my_i,
                    "rgb": rgb,
                    "hsv": hsv_variations
                }, function (e) {
                ...
                let ul_s = false;
                const ul_b = false, lis = [];
                const new_hsv = hsv_variations;

                const gsteps = 100 / gradient_steps;

                ul_s = $("<ul />").addClass("s_variation");
                for (let j = (100 - gsteps); j >= gsteps; j -= gsteps) {
                    ...
                    const param_rgb = HSBToRGB(param_hsv);


                    const s_li = $("<li>").css("background", "#" + RGBToHex(param_rgb)).html("&nbsp;").bind("click", {
                        "value": my_i,
                        "variation": j,
                        "rgb": param_rgb,
                        "hsv": param_hsv
